...
\label{sec:solving}
We now turn our attention to the core component of CRUX and introduce a multi-round adaptive optimization algorithm for identifying querying strategies that maximize the total gain across all rounds under the given budget constraints. As mentioned in the previous section, at each round we assume oracle access to a gain estimator for any generalized query $q^v(k,E)$. Our framework builds upon ideas from the multi-armed bandit literature~\cite{Auer:2003,EvenDar06actionelimination}. Before presenting our framework we list several challenges associated with this adaptive optimization problem.
