...
Other work~\cite{amsterdamer:2014} has studied \agp{XXX?}.
...
As we will see in Section \agp{XXX}, we \agp{ quote a number that summarizes how bad this can be.}

\noindent
{\bf Our approach.} 
\agp{Both this and the subsequent paragraphs have some redundant content. I would collapse into one.}
...
\squishend
\agp{can we quote a couple of performance numbers to indicate how we're doing well so that
the practically minded people can be impressed and so that we indicate that this is
not just an exercise in theory.}
