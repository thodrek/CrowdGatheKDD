...
Motivated by applications as the ones described above we focus on {\em crowdsourced entity extraction}, i.e., the problem of collecting entities for a given domain by asking crowd workers to {\em list entities} from a domain of interest. Due to the cost involved in human labor and the inherent redundancy of crowdsourcing tasks, crowdsourced entity extraction can become highly impractical. In this paper we study techniques for efficient crowdsourced data extraction. Given a monetary budget, our goal is to maximize the number of distinct entities extracted with the minimal number of queries against crowd workers. We use a small pilot experiment, using Amazon's Mechanical Turk~\cite{mturk}, to illustrate why naive crowdsourced entity extraction schemes can be impractical and to motivate our approach. 
\begin{example}
We focused on {\em extracting people from the news} and asked workers to list people of different occupations mentioned in five major online US news portals. 
...
\Cref{tab:duplicates} shows the percentage of entities that were provided by at least two different workers for different $<$occupation, news portal$>$ combinations. As shown, on average, more than half of the extracted entities were reported by at least two workers, while there are cases where the percentage of duplicated entities can be as high as 77\%. This means that for our simple {\em static querying strategy} that issues the same fixed queries to the workers, 77\% of the cost (in compensating workers for their answers) is simply wasted. Applying crowdsourcing naively leads to a large number of {\em repeated extractions of the popular entities, and low coverage of the less-popular entities}.
In fact, the not-so-popular entities may only be extracted after a very long time (translating to high cost and effort) or may never be extracted at all.

\noindent {\bf Limiting Duplicate Extractions.} To remedy this issue, in this paper, we propose to leverage two insights, translating to more {\em fine-grained queries} to the crowd:
...
%\agp{I left this paragraph in as is, but I think this is a bit defensive: also this is too long to describe now -- let's try to get to the meat soon.}
...
We also extend typical crowd queries to include an {\em exclude list}, e.g., ``list a person in the New York Times that is not Donald Trump''.  Excluding popular entities can help us identify more distinct entities much faster.
