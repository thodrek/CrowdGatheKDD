...
\end{theorem}
The proof is provided in \Cref{sec:prth1} and is based on a reduction from the {\em unbounded knapsack} problem. The latter is a variation of the original 0-1 knapsack problem that places no upper bound on the number of copies of each kind of item. The full proof is available in the supplementary material~\cite{cruxsup}.
